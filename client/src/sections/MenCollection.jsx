...
const MenCollection = () => {
  ...
  const fetchCategoryProducts = async () => {
    try {
      setLoading(true)
      let response = await axios.get(`${backendUrl}/api/product/category-products/${'Men'}`, { withCredentials: true })
      if (response.data) {
    ...
    <>
      ...
      <section className="pt-16 lg:pt-20">
        <div className="lg:px-12 md:px-10 sm:px-8 px-5">
          {/* Header */}
          <div>
            <Heading heading='Men Collection' />
          </div>
...
export default React.memo(MenCollection);