...
export const signup = async (req, res) => {
    try {
        ...
        if (name.length < 3 || name.length > 40) {
            return res.status(400).json({
                success: false,
                message: "Name must contain 3–40 characters"
            });
        }

        if (email.length < 12 || email.length > 60) {
            return res.status(400).json({
                success: false,
                message: "Email must contain 12–60 characters"
            });
        }
        ...
        if (password.length < 8 || password.length > 80) {
            return res.status(400).json({
                success: false,
                message: "Password must contain 8–80 characters"
            });
        }
